...
void array_to_png (char* filename, int size, array2<Complex> &field, int frame)
{
  ...
  png.plot_text("/usr/share/X11/fonts/TTF/luxisr.ttf",10,10,10,0.0,text,1.0,1.0,1.0);
  png.close();
}
...
main (void)
{
  ...
  for (intstep = 0; intstep < integrateSteps; intstep++)
  {
    ...
    { 
      ...
      */
      // save the fwd probe and conjugate regions
      if (zindex == NZ-2 && ((intstep+1) % (NZ-1) == 0))
      {
        cout << "saving farfield" << endl;
        for (i = 0; i < N; i++) 
        ...
        sprintf(fwdfilename, "%03i_fwd.dat", transitsCompleted);
        //ofstream fwdfile(fwdfilename);
        fwdfile.open(fwdfilename);
        if (!fwdfile)
        {
          cout << "can't open fwdfile\n";
          return(1);
        }
        for (i = 115; i < 143; i++)
        {
          for (j = 123; j < 134; j++)
          {
                fwdfile << real(farfield(i,j)*conj(farfield(i,j))) << "\t";
          }
          fwdfile << endl;
        }
        fwdfile.close();
